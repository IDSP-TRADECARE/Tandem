    ...
    if (status === 'recording') {
      ...
      mediaRecorderRef.current.onstop = async () => {
        ...
        try {
          ...
          // DETECT NEXT WEEK
          const isNextWeek = detectNextWeek(transcript);
          console.log('📅 Next-week detected:', isNextWeek);

          // COMPUTE CORRECT WEEK START
          ...
          //  VALIDATION FOR VOICE INPUT
          for (const day of days) {
            const sched = schedule.daySchedules[day];

            if (!sched.timeFrom || !sched.timeTo) {
              throw new Error(`Missing times for ${day}`);
            }

            // Normalize (your parser already outputs HH:MM, but to be safe)
            const from = sched.timeFrom.trim();
            const to = sched.timeTo.trim();
            ...
            if (start === end) {
              throw new Error(
                `Start and end times cannot be the same for ${day}`
              );
            }
          ...
          console.log('✅ Voice schedule passed time validation');
          ...
          {status === 'recording' && (
            <div className="text-center">
              ...
              <div className="max-w-md mx-auto">
                ...
                <p className="text-gray-700 text-lg leading-relaxed">
                  Click the button again when finished
                </p>
              </div>
            </div>
          )}
